...
> **Version 5 · 2026-09-05 · supersedes v4 (2026-09-05)**
> *Changed in this version:* **Bruno's and Gus's token blocks are approved**, so the art half of the Episode
> 004 block is discharged and the queue has grown rather than shrunk. §0 and §6 rewritten; the theft ruling
> is now the *only* thing blocking 004. Canon is `series-bible_v6.md`.
...
| **8** | **Rule on the theft double-booking** | **Now the only thing blocking Episode 004.** §2. |
...
- [ ] **Theft is double-booked.** Bible §9 ties it to Arc A and the Watch; the bench arc wants it as Season
      Two's first case. It cannot be both. **This is now the ONLY thing blocking Episode 004** — Bruno's
      tokens closed the other half on 2026-09-05.
...
| **004 — Arc A, the Watch** | Two clean Movement cards now exist (002 and 003), so that condition is met. Bruno's tokens are approved, so **the script is blocked on one thing only: the theft ruling.** His sheet blocks the *art*, not the writing. Bruno does not repent. |
...
**One thing, and it is a story ruling: the theft double-booking.** Arc A wants it; the Season Two bench arc
wants it as its first case. It cannot be both, and 004 *is* Arc A. No amount of art moves this.

~~**Bruno has never been drawn.**~~ **Half closed 2026-09-05.** His token block is approved and canon
(`series-bible_v6.md` §3), so the *script* is unblocked — he can be written and named. What remains is
`bruno-sheet.png`, which blocks the **art** only, and it is `ART-QUEUE.md` §1 item 3 with a finished paste
block. Roll it before the episode art, not alongside it (rule 9).

Gus Thornapple's block was approved in the same sitting. He blocked nothing, but he had been in three
episodes and drawn in none, and Episode 003 had to write a Marketrow crowd generically to keep him out of
frame. That is now fixed at the canon end.
