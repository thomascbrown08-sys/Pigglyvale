...
> **Version 5 · 2026-09-05 · supersedes v4 (2026-09-02)**
> *Changed in this version:* Episode 003, *Quantity Not Stated*, shipped. No corrections to v4 — everything
> below is addition.
> **(1)** §6 ledger row and §6b Season Clock row for 003.
> **(2)** §7 — nine new established facts, the largest of them **Miss Quill's rewritten grant rule**, which
> is now on the page rather than pending.
> **(3)** §8 — three running gags added, one updated.
> **(4)** §5b — pipeline rule 21, on characters who are in the prose and cannot be in the art.
> **(5)** §10 — two Open Threads closed (Thomas's 002 correction, the grant rule), three opened.
> **(6)** §11 added: the art gap that now blocks Episode 004.
>
...
## 11. The art gap, and it now blocks something

**Gus Thornapple has no appearance tokens and no reference image.** So does **Bruno "Buckets" Marrow**.
Both were in the prose of Episode 003 and neither could be in a single frame of its art (see pipeline
rule 21).

This was free in 003. It is **not free in 004**, which is Arc A — the Watch — and Bruno is the lead. An
episode cannot have its hero absent from its own hero shot.

| | Owed | Blocks |
|---|---|---|
| **Bruno "Buckets" Marrow** | token block, then `bruno-sheet.png` and `bruno-portrait.png` | **Episode 004. Hard block.** |
| **Gus Thornapple** | token block, then a sheet | Nothing yet. He has been in three episodes and drawn in none, and that will start to show. |

Write the token blocks the way Ambrose's and Quill's were written — **positive shape language, not species
names alone** (rule 17) — and roll the sheets *before* the episode art, not alongside it (rule 9).

Both are already listed in `prompts/ART-QUEUE.md` §3, which is the operational file; this section exists so
that anybody reading canon rather than the queue finds out too.
