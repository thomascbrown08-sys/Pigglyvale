...
> *Changed in this version:* **the theft double-booking is resolved, so nothing blocks Episode 004 at all.**
> Two more §2 decisions close with it — the Watch's name, settled by consequence, and the theft booking
> itself. §0 and §6 rewritten. Canon is `series-bible_v7.md`; planning is `docs/the-arc-room_v3.md`.
...
| **5** | ~~Write Bruno's token block~~ | **Done 2026-09-05.** Approved with Gus's in the same sitting. Both canon in `series-bible_v6.md` §3. |
| **6** | **Roll `bruno-sheet.png`** | `ART-QUEUE.md` §1 item 3. **The priority roll** — he is the lead of Episode 004. |
| **7** | **Roll Episode 003's six images** | `prompts/ep-003-prompts.md`. The page is live and complete; only the art is outstanding. |
| **8** | ~~Rule on the theft double-booking~~ | **Done 2026-09-05.** One thread across two seasons; both bookings kept. `docs/the-arc-room_v3.md`. |
| **9** | **Write Episode 004** | **Nothing blocks it.** Arc A, the Watch, Bruno leads. The one beat it must not miss: the key does not go back. |

**Nothing is blocking anything.** Episode 004 can be written in the next session, and its art can be rolled
in any order relative to that.

**The work has split cleanly in two and the halves do not need each other.**

**Art, and none of it needs a writing session:** Bruno's sheet, Episode 003's six slots, Gus's sheet, the
Ambrose-and-Quill pair. Four jobs, all with finished paste blocks, all in `prompts/ART-QUEUE.md` §1 and
`prompts/ep-003-prompts.md`.

**Writing, and none of it needs art:** the theft ruling, then Episode 004 itself, which can be written
against the approved tokens whether or not the sheets exist yet.
...
- [x] ~~**Theft is double-booked.**~~ **Closed 2026-09-05.** A false conflict: it is one thread across two
      seasons, not two episodes competing for one plot. 004 carries the offence, the unilateral pardon and
      the withheld key; the Season Two bench carries the reoffence, the hearing and the restitution. Bruno
      is the bench's **third or fourth** case, never its opener. **Ruling stands or falls on one beat: 004
      must not restore the key.** `docs/the-arc-room_v3.md`, *Arc A · The theft thread*.
...
- [x] ~~**"The Watch" as a name.**~~ **Settled 2026-09-05 by consequence, without being decided directly.**
      A bench ruling restitution rather than punishment makes the Watch **protective**, which is what the
      word already means. The name stands; the find-and-replace is retired.
...
| **004 — Arc A, the Watch** | **Unblocked.** Two clean Movement cards exist, Bruno's tokens are approved, and the theft thread is ruled. Carries the offence, the pardon, and **the key that does not go back**. Bruno does not repent; the reoffence and the restitution belong to Season Two. |
...
## 6. Episode 004 — nothing is waiting on anything

Both blocks cleared on 2026-09-05. ~~Bruno's tokens~~ approved; ~~the theft ruling~~ made.

**What 004 must carry, in one line each.** Full spine in `docs/the-arc-room_v3.md` §Arc A.

- **The key does not go back.** This is the hinge of a two-season thread and the single most load-bearing
  beat in the episode. Carolyn forgives — really, immediately, unilaterally — and does **not** restore
  access. Forgive and hand the key back, and Season Two's reoffence indicts her judgment. Forgive and set
  the Watch, and it vindicates her.
- **Bruno does not repent.** No repair beat between them. The episode resolves *in her* and stays unresolved
  *in the relationship.* His apology is a season late, and that is the point.
- **The teaching lands in public, not in her head.** Gus says she is letting him off, somebody else says she
  is being hard, and she has to say the difference out loud to other people.
- **Romans 12:18**, with both qualifiers intact — *if possible*, and *so far as it depends on you*.
- **Forgiveness stays unilateral.** Nothing may imply she withholds it pending his repentance. Trust is what
  is conditional; reconciliation is what needs two.

**Art is independent of the script.** `bruno-sheet.png` blocks Episode 004's *illustrations* and nothing
else — the episode can be written against his approved tokens whether or not the sheet exists yet.
`ART-QUEUE.md` §1 item 3.
